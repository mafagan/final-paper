...
\section{决断集模版的生成与验证}	为了避免出现决断集模板基数无限增大的情况，我们需要的决断集模板的替换做出进一步限制。在实例化替换的基础上，我们提出差异化实例替换。差异化实例替换不仅需要满足条件所有的个体变量都被映射到个体实例，还需要满足条件对于所有的不相同变量，被映射后的个体也不相同。直观地，一个差异化实例替换会是变量到个体间的一一映射。同时，一般地一个决断集模板需要能够生成至少一个决断集，然而为了保证最后生成的解释的合理性，我们需要决断集模板的所有差异化实例替换都只生成决断集，因此我们需要限制所有的对于$\alpha_{(x, y)}$决断集模板，在差异化实例替换的映射下能够维持蕴含$\alpha_{(x,y)}\theta$ 这一特性。因此我们对决断集模板做出如下定义：
	...
	为了保证决断集能够被映射到决断集空间上，我们使用从决断集上生成决断集模板的方法来获取决断集模板。我们在生成的决断集的基础上，对二元关系中的个体进行变量替换，且对于不相同的个体名，我们使用不同的个体变量替换。我们对用不相同变量替换不相同个体的过程记作$lift(S,\mathsf{A},\mathsf{B})$。$lift(S,\mathsf{A},\mathsf{B})$ 表示对公理集合中的所有个体名或是角色与概念，我们把$\mathsf{A}$ 映射为变量$X$, $\mathsf{B}$映射为变量$Y$, 其它不相同的个体或是角色与概念分别映射到不同的角色与概念变量。
	\begin{proposition}
...
\section{基于决断集模版的诊断}	
    ...
    在本节中，我们主要介绍生成基于决断集模版的诊断方法。在命题3.4中我们给出了基于决断集模板解释集合的描述，对生成解释的验证方法可以通过简化非平凡、$\subseteq_\mathsf{ds}$-$\emph{minimal}$以及可接受特性的测试方法达到优化的目的，因此我们有以下推论。
     ...
     \begin{corollary}
    给定一个一致的本体$\mathcal{O}$，一个观察值$\alpha_{(\mathsf{A}, \mathsf{B})}$以及命题3.4中给定的决断集模板的集合$\mathcal{P}$，根据决断集模板的集合$\mathcal{P}$在$\mathcal{O}$中得到的对于观察值$\alpha_{(\mathsf{A}, \mathsf{B})}$的基于决断集模板的解释的集合为$\{\emph{\rm{inst}}(\mathcal{J}_1\theta) \mid \mathcal{J}_p \in \mathcal{P}, (\mathcal{J}_1, \mathcal{J}_2) \in \mathsf{B}^*(\mathcal{J}_p)$ 且$\theta$ 是$\mathcal{J}_2 \cup \alpha_{(X, Y)}$的一个差异化实例替换使得$\emptyset \subset \mathcal{J}_2 \subset \mathcal{J}_P$, $X\theta=A$，$Y\theta=\mathsf{B}$，$\mathcal{J}_2\theta \subset \mathcal{O}$,$\mathcal{O} \cup \rm{inst}(\mathcal{J}_1\theta)$是一致的，对于所有的$\mathcal{J} \in \mathsf{subs_1}(\mathcal{J}_2\theta)$有$\mathcal{J} \cup \rm{inst}(\mathcal{J}_1\theta)\not \models \alpha_{(\mathsf{A}, \mathsf{B})}$，以及对于所有的$\mathcal{E}'\in \mathsf{lifts_1}(\rm{inst}(\mathcal{J}_1\theta))\cup \mathsf{subs_1}(\rm{inst}(\mathcal{J}_1\theta))$使得$\mathcal{O}\cup\mathcal{E}'\not\models\alpha_{(\mathsf{A}, \mathsf{B})}$。
	\end{corollary}

     证明：本推论的证明由以下四点组成。

     (1)当对于所有的$\mathcal{J}\in\mathsf{subs_1}(\mathcal{J_2}\theta)$使得$\mathcal{J}\cup \rm{inst}(\mathcal{J}_1\theta)\not\models\alpha_{(\mathsf{A}, \mathsf{B})}$，我们有对于所有的$\mathcal{J} \subset \mathcal{J}_2\theta$，$\mathcal{J}\cup \rm{inst}(\mathcal{J}_1\theta)\not \models\alpha_{(\mathsf{A}, \mathsf{B})}$成立，所以$\rm{inst}(\mathcal{J}_1\theta)\not \models\alpha_{(\mathsf{A}, \mathsf{B})}$。

     (2)存在一个$\mathcal{J}_P$的差异化替换$\theta'$使得$X\theta'=X\theta'$，$Y\theta=Y\theta'$，$\mathcal{J}_1\theta'=\rm{inst}(\mathcal{J}_1\theta)$ 且$\mathcal{J}_2\theta'=\mathcal{J}_2\theta$。由于$\mathcal{J}_2\theta\cup \rm{inst}(\mathcal{J}_1\theta)=\mathcal{J}_P\theta'$以及$\mathcal{J}_P\theta'\models X\theta' \sqsubseteq Y\theta'$，我们有$\mathcal{J}_2\theta\cup \rm{inst}(\mathcal{J}_1\theta)\models\alpha_{(\mathsf{A}, \mathsf{B})}$。所以，当$\mathcal{J}_2\theta \subseteq \mathcal{O}$以及对于所有的$\mathcal{J}\in \mathsf{subs_1}(\mathcal{J}_2\theta)$，我们有$\mathcal{J}_2\theta\cup \rm{inst}(\mathcal{J}_1\theta)\in \mathsf{Jst}(\alpha_{(\mathsf{A}, \mathsf{B})},\mathcal{O}\cup \rm{inst}(\mathcal{J}_1\theta))$。另一方面，当$\mathcal{J}_2\theta\cup \rm{inst}(\mathcal{J}_1\theta)\in\mathsf{Jst}(\alpha_{(\mathsf{A}, \mathsf{B})}, \mathcal{O}\cup \rm{inst}(\mathcal{J}_1\theta))$，可得对于所有的$\mathcal{J} \in \mathsf{subs_1}(\mathcal{J}_2\theta)$有$\mathcal{J}\cup \rm{inst}(\mathcal{J}_1\theta)\not \models\alpha_{(\mathsf{A}, \mathsf{B})}$。

     (3)通过命题3.3，对于所有的$\mathcal{E}'\in\mathsf{lifts_1}(\rm{inst}(\mathcal{J}_1\theta))\cup\mathsf{subs_1(\rm{inst}(\mathcal{J}_1\theta))}$ 有$\mathcal{O}\cup\mathcal{E}'\not\models\alpha_{(\mathsf{A}, \mathsf{B})}$当且仅当不存在一个对于$\alpha_{(\mathsf{A}, \mathsf{B})}$的解释$\mathcal{E}'$使得$\mathcal{E}'\subseteq_{ds} \rm{inst}(\mathcal{J}_1\theta)$且$\rm{inst}(\mathcal{J}_1\theta)\not\subseteq_{ds}\mathcal{E}'$。

     (4)通过第(2)条我们可以有$\mathcal{J}_2\theta\cup \rm{inst}(\mathcal{J}_1\theta)\models\alpha_{(\mathsf{A}, \mathsf{B})}$。由于$\rm{inst}(\mathcal{J}_1\theta)\not \models\alpha_{(\mathsf{A}, \mathsf{B})}$，我们有$\mathcal{J}_2 \not= \emptyset$。又由于$\mathcal{J}_2\theta\subseteq\mathcal{O}$，$\mathcal{O}\not\models\alpha_{(\mathsf{A}, \mathsf{B})}$，我们有$\mathcal{J}_1\not=\emptyset$且$\mathcal{J}_2\subset\mathcal{J}_P$。
...
\begin{algorithm}
  \caption{基于决断集模板诊断算法}
  \KwIn{Ontology $\mathcal{O}$, observation $r_o(\mathsf{A},\mathsf{B})$, Justification Pattern  $\mathcal{J}_p$ for $r_o(X,Y)$ }
...
\end{example}
	...
	\right\}.
	$$
	易得，$\mathcal{O}$与$\mathcal{O}\cup r_{Nationality}(\mathsf{\mathsf{Mike}}, \mathsf{\mathsf{China}})$都是一致的。又，通过决断集可以得到蕴含$r_{Nationality}(A, \mathsf{B})$ 的基于决断集模板$\mathcal{J}_p$:
