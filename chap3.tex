...
\section{决断集模版的生成与验证}	为了避免出现决断集模板基数无限增大的情况，我们需要的决断集模板的替换做出进一步限制。在实例化替换的基础上，我们提出差异化实例替换。差异化实例替换不仅需要满足条件所有的个体变量都被映射到个体实例，还需要满足条件对于所有的不相同变量，被映射后的个体也不相同。直观地，一个差异化实例替换会是变量到个体间的一一映射。同时，一般地一个决断集模板需要能够生成至少一个决断集，然而为了保证最后生成的解释的合理性，我们需要决断集模板的所有差异化实例替换都只生成决断集，因此我们需要限制所有的对于$r(e_x, e_y)$决断集模板，在差异化实例替换的映射下能够维持蕴含$r(e_x\theta, e_y\theta)$这一特性。因此我们对决断集模板做出如下定义：
    ...
    一个由扩展公理组成的集合$\mathcal{J}_p$会被称为在本体$\mathcal{O}$中对于$r(e_x\, e_y)$的决断集模版如果$\mathcal{J}_p$满足以下两个条件:(1) 存在一个替换$\theta$使得$\mathcal{J}_p\theta\in \emph{Jst}\left(r\left(e_x\theta, e_y\theta\right), \mathcal{O}\right)$\ \ (2)对于所有的差异化实例替换$\sigma$存在$\mathcal{J}_p\sigma\models r\left(e_x\sigma, e_y\sigma\right)$。
	...
	为了保证决断集能够被映射到决断集空间上，我们使用从决断集上生成决断集模板的方法来获取决断集模板。我们在生成的决断集的基础上，对二元关系中的个体进行变量替换，且对于不相同的个体名，我们使用不同的个体变量替换。我们对用不相同变量替换不相同个体的过程记作$lift(S,A,\mathsf{B})$。$lift(S,A,\mathsf{B})$表示对公理集合中的所有个体名，我们把$A$映射为变量$X$, $\mathsf{B}$映射为变量$Y$, 其它不相同的个体分别映射到不同的个体变量。
	\begin{proposition}
...
\section{基于决断集模版的解释}
	...
	由于解释变量的存在，如果想保证诊断的方法能够更加高效，一种考虑是只计算无法被其它解释通过实例化替换后得到的解释。然而解释变量的允许却会导致解释公理集合基数的变大甚至无法限制，考虑以下解释$r_o(\mathsf{A}, A)$，这个解释可以由解释$\mathcal{E} = \{r_o(\mathsf{A}, e_{x_1}), r_o(e_{x_2}, e_{x_3}), \cdots, r_o(e_{x_{n-1}}, e_n)\}$ 通过映射$\theta = \{e_{x_i} \mapsto A \mid 1 \leq i \leq n\}$获得。因此对于解释的替换，我们提出一个新的概念，叫做解释差异化替换。解释$\mathcal{E}$的差异化替换会可以把$\mathcal{E}$的解释变量映射到不同的解释变量或者是一个没有在$\mathcal{E}$中已经出现的变量。在前文提到的例子中，$\theta$并不是一个解释的差异化替换，因为$e_{x_i}$被映射到的变量$A$是解释$\mathcal{E}$中已经存在的个体实例，所以$\theta$不是解释的差异化替换。因此我们提出一个新的子集概念：
    ...
    \begin{itemize}
        ...
        \item (可容许) 存在一个决断集模版$\mathcal{J}_p \in \mathcal{P}$以及$\mathcal{J}_p$的一个差异化替换$\theta$使得$\alpha = r_o(X\theta, Y\theta)$，$\mathcal{E} \subseteq \mathcal{J}_p\theta$且$\mathcal{J}_p\theta \in Jst(\alpha, \mathcal{O}\cup\mathcal{E})$
    \end{itemize}
\end{definition}
	
	计算基于决断集模板的解释是一个多项式时间内的计算过程。根据观察值$r_o(\mathsf{A},\mathsf{B})$以及决断集模板集合$\mathcal{P}$计算在本体$\mathcal{O}$中的基于决断集模板解释步骤如下：对于所有在$\mathcal{P}$中的决断集模板$\mathcal{J}_p$，我们使用二分法对决断集模板$\mathcal{J}_p$进行切分，其中一部分公理的集合我们把它看做是本体$\mathcal{O}$的对应公理集合，另一部分我们把它看做是我们需要求得的基于决断集模板的解释。给定一个决断集模板$\mathcal{J}_p$，我们定义$\mathcal{J}_p$的二分结果为二元组$(\mathcal{J}_1, \mathcal{J}_2)$，其中$\mathcal{J}_1 \cap \mathcal{J}_2 = \emptyset$且$\mathcal{J}_1 \cup \mathcal{J}_2 = \mathcal{J}_p$。我们把$\mathcal{J}_p$二分后的二元组集合记作$\mathsf{B}^*(\mathcal{J}_p)$。同时，对于一个由多条扩展公理的集合$S$我们把从个体变量映射到个体实例的一一映射操作记作$inst(S)$。因此，我们有如下命题：
	
	\begin{proposition}
    给定一个一致的本体$\mathcal{O}$，一个观察值$r_o(\mathsf{A}, \mathsf{B})$且$\mathcal{O} \not \models r_o(\mathsf{A}, \mathsf{B})$，以及蕴含$r_o(X, Y)$的决断集模板的集合$\mathcal{P}$，那么在本体$\mathcal{O}$中基于决断集模板集合$\mathcal{P}$对于观察值$r_o(\mathsf{A}, \mathsf{B})$ 的基于决断集模板的解释的集合$\mathcal{S} = \{\emph{inst}(\mathcal{J}_1\theta) \mid \mathcal{J}_p \in \mathcal{P}, (\mathcal{J}_1, \mathcal{J}_2) \in \mathsf{B}^*(\mathcal{J}_p)$ 且$\theta$ 是$\mathcal{J}_2 \cup r_o(X, Y)$的一个差异化实例替换使得 $X\theta=A$，$Y\theta=\mathsf{B}$，$\mathcal{J}_2\theta \subseteq \mathcal{O}$, 同时$inst(\mathcal{J}_1\theta) \not \models r_o(X, Y)$，$\mathcal{O} \cup inst(\mathcal{J}_1\theta)$是一致的，且$\mathcal{J}_2\theta\cup \emph{inst}(\mathcal{J}_1\theta)\in \emph{Jst}(r_o(\mathsf{A}, \mathsf{B}), \mathcal{O} \cup \emph{inst}(\mathcal{J}_1)\theta)$,以及不存在一个在本体$\mathcal{O}$对于观察值$r_o(\mathsf{A}, \mathsf{B})$的解释$\mathcal{E}'$满足$\mathcal{E}'\subseteq_\mathsf{ds} \emph{inst}(\mathcal{J}_1\theta)$以及$\emph{inst}(\mathcal{J}_1\theta) \not \subseteq_\mathsf{ds} \mathcal{E}'\}$。
	\end{proposition}
	
	
	证明：(1)令$\mathcal{E}$是集合$\mathcal{S}$中的一个元素，存在一个或多个的$\mathcal{J}_p\in\mathcal{P}$，$(\mathcal{J}_1,\mathcal{J}_2) \in \mathsf{B}^*(\mathcal{J}_p)$以及对于$\mathcal{J}_2 \cup \{r_o(X, Y)\}$的差异化实例替换$\theta$使得$\mathcal{E} = \emph{inst}(\mathcal{J}_1\theta), X\theta=A, Y\theta=\mathsf{B},\mathcal{J}_2\theta \subseteq \mathcal{O}, \mathcal{E} \not \models r_o(\mathsf{A}, \mathsf{B})$，$\mathcal{O} \cup \mathcal{E}$是一致的。$\mathcal{J}_2\theta \in Jst(r_o(\mathsf{A}, \mathsf{B}), \mathcal{O} \cup \mathcal{E})$，并且不存在一个在$\mathcal{O}$中对于$r_o(\mathsf{A}, \mathsf{B})$的解释$\mathcal{E}'$满足$\mathcal{E}'\subseteq_\mathsf{ds} \mathcal{E} $且$\mathcal{E} \not \subseteq_\mathsf{ds} \mathcal{E}'$。由于$\mathcal{E}$是由$\mathcal{J}_1\theta$通过实例化替换而来，因此一定存在一个$\mathcal{J}_p$的实例化替换$\theta'$使得$\mathcal{J}_1\theta' = \mathcal{E}$，$\mathcal{J}_2\theta' = \mathcal{J}_2\theta, X\theta' = X\theta = A$以及$Y\theta' = Y\theta = \mathsf{B}$。通过定义可知，我们有$\mathcal{J}_p\theta' \models r_o(X, Y)$。由于$\mathcal{J}_p\theta' = \mathcal{J}_2\theta \cup \mathcal{E}$并且$\mathcal{J}_2\theta \subseteq \mathcal{O}$，我们有$\mathcal{J}_p\theta \subseteq \mathcal{O} \cup \mathcal{E}$所以$\mathcal{O} \cup \mathcal{E} \models r_o(\mathsf{A}, \mathsf{B})$，$\mathcal{E}$是在本体$\mathcal{O}$中对于观察值$r_o(\mathsf{A}, \mathsf{B})$的一个解释。另外，$\mathcal{E}$满足基于决断集模板解释的四个特性：非平凡(i.e., $\mathcal{E} \not \models \alpha$)，一致(i.e. $\mathcal{O} \cup \mathcal{E}$ 是一致的)以及$\subseteq_\mathsf{ds}-\emph{minimal}$(不存在这样一个解释$\mathcal{E}'$使得$\mathcal{E}'\subseteq_\mathsf{ds}\mathcal{E}$ 且$\mathcal{E}\not\subseteq_\mathsf{ds}\mathcal{E}'$，其中$\mathcal{E}'\subseteq_\mathsf{ds}\mathcal{E}$ 表示存在一个$\mathcal{E}'$的差异化替换$\theta$，使得$\mathcal{E}'\theta\subseteq\mathcal{E}$)，以及可容许(存在一个决断集模版$\mathcal{J}_p \in \mathcal{P}$以及$\mathcal{J}_p$的一个差异化替换$\theta$使得$\alpha = r_o(X\theta, Y\theta)$，$\mathcal{E} \subseteq \mathcal{J}_p\theta$且$\mathcal{J}_p\theta \in Jst(\alpha, \mathcal{O}\cup\mathcal{E})$)，所以$\mathcal{E}$是一个在$\mathcal{O}$中对于$r_o(\mathsf{A}, \mathsf{B})$ 的一个基于决断集模板的解释。(2)令$\mathcal{E}$为在本体$\mathcal{O}$中基于决断集模板$\mathcal{P}$对观察值$r_o(\mathsf{A},\mathsf{B})$的解释。由于$\mathcal{E}$是一个可接受的解释，因此必定存在一个决断集模板$\mathcal{J}_p \in  \mathcal{P}$以及一个$\mathcal{J}_p$的差异化实例替换$\theta$使得$X\theta=A, Y\theta=\mathsf{B}, \mathcal{E}\subseteq \mathcal{J}_p\theta$以及$\mathcal{J}_p\theta \in Jst(r_o(\mathsf{A},\mathsf{B}), \mathcal{O}\cup \mathcal{E})$。由于$\mathcal{E}\subseteq \mathcal{J}_p\theta$以及$\mathcal{J}_p\theta \subseteq $，所以必定存在一个$\mathcal{J}_p$的二分$(\mathcal{J}_1, \mathcal{J}_2)$使得$\mathcal{E}=\mathcal{J}_1\theta$以及$\mathcal{J}_2\theta \subseteq \mathcal{O}$。令$\theta'$是一个$\mathcal{J}_2 \cup \{r_o(\mathsf{A},\mathsf{B})\}$使得$X\theta'=X\theta, Y\theta'=Y\theta$以及$\mathcal{J}_2\theta'=\mathcal{J}_2\theta$，所以$X\theta'A=A,Y\theta'=\mathsf{B},\mathcal{J}_2\theta' \subseteq \mathcal{O}$以及$\mathcal{E}$是$\emph{inst}(\mathcal{J}_1\theta')$的重名等价解。又一次$\mathcal{E}$是非平凡的，我们有$\emph{inst}(\mathcal{J}_1\theta')\not \models r_o(\mathsf{A},\mathsf{B})$。$\mathcal{E}$是一致的，因此$\mathcal{O} \cup \emph{inst}(\mathcal{J}_1\theta')$是一致的。$\mathcal{J}_2\theta \cup \mathcal{E} \in Jst(r_o(\mathsf{A},\mathsf{B}), \mathcal{O}\cup \mathcal{E})$，我们有$\mathcal{J}_2\theta' \cup \emph{inst}(\mathcal{J}_1\theta') \in Jst(r_o(\mathsf{A},\mathsf{B}), \mathcal{O} \cup \emph{inst}(\mathcal{J}_1\theta'))$。又$\mathcal{E}$是$\subseteq_\mathsf{ds}-\emph{minimal}$的，因此不存在一个在本体$\mathcal{O}$对于观察值$r_o(\mathsf{A},\mathsf{B})$的解释$\mathcal{E}'$ 使得$\mathcal{E}'\subseteq_\mathsf{ds} \emph{inst}(\mathcal{J}_1\theta')$并$\emph{inst}(\mathcal{J}_1\theta')  \not \subseteq_\mathsf{ds}\mathcal{E}'$

    在上文中，我们已经证明了基于决断集模板的解释可以在一个有限的空间中形成。

\section{生成基于决断集模版解释的算法}	
	
    在本节中，我们主要介绍生成基于决断集模版解释的算法。给定训练集，观察值$r_o(\mathsf{A},\mathsf{B})$，以及蕴含$r_o(X,Y)$的决断集模版的集合$\mathcal{P}$，算法2计算出所有相应的基于决断集模版的解释集。对于给定的输入，算法2首先检测表达本体$\mathcal{O}$的一致性，之后对于所有的决断集模版$\mathcal{J}_p$，我们使用使用二分法生成$(\mathcal{J}_1, \mathcal{J}_2)$。其中$\mathcal{J}_1$用于在$\mathcal{O}\cup r_o(\mathsf{A},\mathsf{B})$中寻找实例并计算出替换$\theta$，$\mathcal{J}_2$用于生成解释并验证是否合理解释。遍历所有的$(\mathcal{J}_1, \mathcal{J}_2)$(除了$\mathcal{J}_1 = \emptyset$或$\mathcal{J}_2 = \emptyset$)得出解释集。例3.4是根据算法给出的例子。
...
\end{example}
	...
	对决断集模板进行二分划分$(\mathcal{J}_1, \mathcal{J}_2)$，使得$\mathcal{J}_1 \cap \mathcal{J}_2 = \emptyset$且$\mathcal{J}_1 \cup \mathcal{J}_2 = \mathcal{J}_p$。
        \begin{center}
	    \item 1)\ $J_1$= $\{r_{BornInCity}(\mathsf{A}, \mathsf{C}), r_{CityInProvince}(\mathsf{C}, \mathsf{D})\}$, $J_2$=$\{r_{ProvinceInCountry}(\mathsf{D}, \mathsf{B})\}$
		\end{center}
		由于不存在一个差异化实例替换$\theta$使得$\mathcal{J}_1\theta \subseteq \mathcal{O}$，因此划分1)无法生成合理解释。
		\begin{center}
        \item 2)\ $J_1$ $\{r_{BornInCity}(\mathsf{A}, \mathsf{C})\}$, $J_2$=$\{r_{CityInProvince}(\mathsf{C}, \mathsf{D}), r_{ProvinceInCountry}(\mathsf{D}, \mathsf{B})\}$
		\end{center}
		根据划分2)，可得$\theta$使得 $\mathcal{J}_1\theta \subseteq \mathcal{O}$，其中$\theta = \{\mathsf{A} \mapsto \mathsf{Mike}, \mathsf{C} \mapsto \mathsf{Hangzhou}, \mathsf{B} \mapsto \mathsf{China}\}$，设$\mathcal{E}_2 = \mathcal{J}_2\theta = \{r_{CityInProvince}(\mathsf{Hangzhou}, e_1), r_{ProvinceInCountry}(e_1, \mathsf{China})\}$，又因为$\mathcal{O} \cup \mathcal{J}_2\theta$是一致的，$\mathcal{O} \cup \mathcal{J}_2\theta \models \alpha$，且对于所有的$\mathcal{E}' \in subs_1(\mathcal{E}_2) \cup lifts_1(\mathcal{E}_2)$都有$\mathcal{O} \cup \mathcal{E}' \not \models \alpha$，所以$\mathcal{E}_2$是一个合理的解释。
		
		\begin{center}
	    \item 3)\ $J_1$= $\{r_{BornInCity}(\mathsf{A}, \mathsf{C}), r_{ProvinceInCountry}(\mathsf{D}, \mathsf{B})\}$, $J_2$=$\{r_{CityInProvince}(\mathsf{C}, \mathsf{D})\}$
		\end{center}
		根据划分3)，可以得到一个$\theta$使得 $\mathcal{J}_1\theta \subseteq \mathcal{O}$，其中$\theta = \{\mathsf{A} \mapsto \mathsf{Mike}, \mathsf{C} \mapsto \mathsf{Hangzhou}, \mathsf{B} \mapsto \mathsf{China}, \mathsf{D} \mapsto \mathsf{Zhejiang}\}$，所以我们设$\mathcal{E}_3 = \mathcal{J}_2\theta = \{r_{CityInProvince}(\mathsf{Hangzhou}, \mathsf{Zhejiang})\}$，又因为$\mathcal{O} \cup \mathcal{J}_2\theta$是一致的，$\mathcal{O} \cup \mathcal{J}_2\theta \models \alpha$，且对于所有的$\mathcal{E}' \in subs_1(\mathcal{E}_2) \cup lifts_1(\mathcal{E}_3)$都有$\mathcal{O} \cup \mathcal{E}' \not \models \alpha$，所以$\mathcal{E}_3$是一个合理的解释。
		
		
		\begin{center}
	    \item 4)\ $J_1$=$\{r_{ProvinceInCountry}(\mathsf{D}, \mathsf{B})\}$, $J_2$= $\{r_{BornInCity}(\mathsf{A}, \mathsf{C}), r_{CityInProvince}(\mathsf{C}, \mathsf{D})\}$
		\end{center}
		根据划分4)，可以得到一个$\theta$使得 $\mathcal{J}_1\theta \subseteq \mathcal{O}$，其中$\theta = \{\mathsf{A} \mapsto \mathsf{Mike}, \mathsf{B} \mapsto \mathsf{China}, \mathsf{D} \mapsto \mathsf{Zhejiang}\}$，所以我们设$\mathcal{E}_4 = \mathcal{J}_2\theta = \{r_{BornInCity}(\mathsf{Mike}, e_1), r_{CityInProvince}(e_1, \mathsf{Zhejiang})\}$，又因为$\mathcal{O} \cup \mathcal{J}_2\theta$是一致的，$\mathcal{O} \cup \mathcal{J}_2\theta \models \alpha$，且对于所有的$\mathcal{E}' \in subs_1(\mathcal{E}_4) \cup lifts_1(\mathcal{E}_4)$都有$\mathcal{O} \cup \mathcal{E}' \not \models \alpha$，所以$\mathcal{E}_4$是一个合理的解释。
		
		\begin{center}
	    \item 5)\ $J_1$=$\{r_{CityInProvince}(\mathsf{C}, \mathsf{D}), r_{ProvinceInCountry}(\mathsf{D}, \mathsf{B})\}$, $J_2$ $\{r_{BornInCity}(\mathsf{A}, \mathsf{C})\}$
		\end{center}
		根据划分5)，可以得到一个$\theta$使得 $\mathcal{J}_1\theta \subseteq \mathcal{O}$，其中$\theta = \{\mathsf{A} \mapsto \mathsf{Mike}, \mathsf{C} \mapsto \mathsf{Beihai}, \mathsf{B} \mapsto \mathsf{China}, \mathsf{D} \mapsto \mathsf{Guangxi}\}$，所以我们设$\mathcal{E}_5 = \mathcal{J}_2\theta = \{r_{BornInCity}(\mathsf{Mike}, \mathsf{Beihai})\}$，又因为$\mathcal{O} \cup \mathcal{J}_2\theta$是一致的，$\mathcal{O} \cup \mathcal{J}_2\theta \models \alpha$，且对于所有的$\mathcal{E}' \in subs_1(\mathcal{E}_5) \cup lifts_1(\mathcal{E}_5)$都有$\mathcal{O} \cup \mathcal{E}' \not \models \alpha$，所以$\mathcal{E}_5$是一个合理的解释。。
		
		\begin{center}
	    \item 6)\ $J_2$=$\{r_{CityInProvince}(\mathsf{C}, \mathsf{D})\}$, $J_1$= $\{r_{BornInCity}(\mathsf{A}, \mathsf{C}), r_{ProvinceInCountry}(\mathsf{D}, \mathsf{B})\}$
		\end{center}
		根据划分6)，可以得到一个$\theta$使得 $\mathcal{J}_1\theta \subseteq \mathcal{O}$，其中$\theta = \{\mathsf{A} \mapsto \mathsf{Mike}, \mathsf{C} \mapsto \mathsf{Beihai}, \mathsf{B} \mapsto \mathsf{China}, \mathsf{D} \mapsto \mathsf{Guangxi}\}$，所以我们设$\newline$
		\begin{center}
		\item $\mathcal{E}_6 = \mathcal{J}_2\theta = \{r_{BornInCity}(\mathsf{Mike}, \mathsf{Beihai}), r_{ProvinceInCountry}(\mathsf{Guangxi}, \mathsf{China})\}$
		\end{center}
